...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted">Update blog post details</p>
    </div>
    <div>
        <a href="{{ route('admin.posts.show', $post) }}" class="btn btn-info me-2">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">Upload a new image to replace the current one (JPEG, PNG, JPG, GIF, SVG). Max size: 2MB</div>
                            </div>
                        </div>
                    </div>
                </div>
...
@endpush
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <a href="{{ route('admin.posts.index') }}" class="btn btn-secondary ms-2">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    <label for="status" class="form-label">Status <span class="text-danger">*</span></label>
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" form="postForm">
                        <option value="draft" {{ old('status', $post->status) === 'draft' ? 'selected' : '' }}>Draft</option>
                        <option value="published" {{ old('status', $post->status) === 'published' ? 'selected' : '' }}>Published</option>
                    </select>
                    @error('status')
                ...
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured"
                           value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} form="postForm">
                    <label class="form-check-label" for="is_featured">
                        Featured Post
                    </label>
                </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    <label for="author_id" class="form-label">Author <span class="text-danger">*</span></label>
                    <select class="form-select @error('author_id') is-invalid @enderror" id="author_id" name="author_id" form="postForm" required>
                        <option value="">Select Author</option>
                        @foreach($authors as $author)
                        <option value="{{ $author->id }}" {{ old('author_id', $post->author_id) == $author->id ? 'selected' : '' }}>
                            {{ $author->name }}
                ...
                <div class="mb-3">
                    <label for="category_id" class="form-label">Category <span class="text-danger">*</span></label>
                    <select class="form-select @error('category_id') is-invalid @enderror" id="category_id" name="category_id" form="postForm" required>
                        <option value="">Select Category</option>
                        @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $post->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
...
// Set all form elements to belong to the main form
document.addEventListener('DOMContentLoaded', function() {
    const formElements = document.querySelectorAll('[form="postForm"]');
